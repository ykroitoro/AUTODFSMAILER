...
start_row = 17
start_col = 8  # Start from column A
...
def write_summary_to_menu(ws, df, sheet_name, start_row, start_col):
    ...
    ws.merge_cells(start_row=15, start_column=8, end_row=15, end_column=20)
    ws.cell(15,8, value ="SUMMARY TABLES BY CATEGORY, PROCESSOR AND FORM FACTOR")
    ws.cell(15,8).font = Font(bold=True, size=20, color="FFFFFF")  # Set font size to 14 and bold
    ws.cell(15,8).fill = PatternFill(start_color="000080", end_color="000080", fill_type="solid")
    ws.cell(15,8).alignment = center_alignment
...
for sheet_name in wb.sheetnames:
    if sheet_name != 'MENU':
        ...
        if 'CPU' in df.columns and 'Form Factor' in df.columns and 'Qty' in df.columns:
            print(f"Creating summary for {sheet_name}")
            
            # Create summary table grouped by CPU and Form Factor, with total qty
            summary_df = df.groupby(['CPU', 'Form Factor']).agg({'Qty': 'sum'}).reset_index()
