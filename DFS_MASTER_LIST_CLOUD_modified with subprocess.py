...
def run_master_list_process():
    ...
    for sheet_name in wb.sheetnames:
        if sheet_name != 'MENU':
            ...
            def run_master_list_process():
                print("🚀 Starting run_master_list_process")
                ...
                try:
                    ...
                    # ✅ Save directly to local Dropbox-synced folder
                    OUTPUT_PATH = os.path.expanduser(f"~/Dropbox/AUTODFSMAILER/LISTS/{OUTPUT_FILENAME}")

                    # ✅ Save workbook directly to disk
                    menu_wb.save(OUTPUT_PATH)
                    print(f"📥 Saved Excel file locally at: {OUTPUT_PATH}")
                    ...
                    print(f"❌ Fatal error in run_master_list_process: {e}")
                    raise

            if __name__ == "__main__":
                try:
                    path = run_master_list_process()
                    print(f"✅ File saved to: {path}")
                except Exception as e:
                    print(f"❌ Error running DFS_MASTER_LIST_CLOUD.py: {e}")
